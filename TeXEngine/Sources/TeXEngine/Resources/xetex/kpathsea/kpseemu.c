...
string concat3_noexit(const_string s1, const_string s2, const_string s3) {
  int s2l = s2 ? strlen(s2) : 0;
  int s3l = s3 ? strlen(s3) : 0;
  int new_size = strlen(s1) + s2l + s3l + 1;
  string answer = (string)malloc(new_size);
  if (!answer) {
    return NULL;
  }
  memset(answer, 0, new_size);
  strcpy(answer, s1);
  if (s2)
    strcat(answer, s2);
  if (s3)
...
char *get_current_work_dir(void) {
...
char* kpse_find_file(const char* name, kpse_file_format_type format,
  ...
  char *current_dir = get_current_work_dir();
    if (current_dir)
    {
        char *new_path = concat3(current_dir, name, NULL);
  ...
  if (strstr(basePath, ".") == NULL) {
    ...
    char *new_path_2 = concat3(current_dir, local_name, NULL);
    if  (access(new_path_2, F_OK) != -1) {
        if (current_dir) {
                free(current_dir);
        }
        return new_path_2;
