...
find_post (void)
{
    ...
    if ((ch = tt_dpx_get_unsigned_byte(dvi_handle)) != PRE) {
        dpx_message("Found %d where PRE was expected\n", ch);
        _tt_abort(invalid_signature);
    }
    ch = tt_dpx_get_unsigned_byte(dvi_handle);
...
get_page_info (int32_t post_location)
{
    ...
    num_pages = tt_dpx_get_unsigned_pair(dvi_handle);
    if (num_pages == 0) {
...
get_dvi_info (int32_t post_location)
{
    ...
    dvi_info.stackdepth   = tt_dpx_get_unsigned_pair(dvi_handle);
...
read_font_record (uint32_t tex_id)
{
    ...
    dir_length  = tt_dpx_get_unsigned_byte(dvi_handle);
    name_length = tt_dpx_get_unsigned_byte(dvi_handle);
...
read_native_font_record (uint32_t tex_id)
{
    ...
    point_size  = tt_get_positive_quad(dvi_handle, "DVI", "point_size");
    flags       = tt_dpx_get_unsigned_pair(dvi_handle);

    len = (int) tt_dpx_get_unsigned_byte(dvi_handle); /* font name length */
...
get_dvi_fonts (int32_t post_location)
{
    ...
    while ((code = tt_dpx_get_unsigned_byte(dvi_handle)) != POST_POST) {
        switch (code) {
...
static void get_comment (void)
{
    ...
    length = tt_dpx_get_unsigned_byte(dvi_handle);
                        ...
                        spt_t ptsize, int layout_dir, int extend, int slant, int embolden)
{
    ...
    fontmap_key = dpx_malloc(strlen(filename) + 40); // CHECK this is enough
...
skip_fntdef (void)
{
    ...
    area_len = tt_dpx_get_unsigned_byte(dvi_handle);
    name_len = tt_dpx_get_unsigned_byte(dvi_handle);
...
check_postamble (void)
{
    ...
    tt_skip_bytes(4, dvi_handle);
    post_id_byte = tt_dpx_get_unsigned_byte(dvi_handle);
...
dvi_do_page (double page_paper_height, double hmargin, double vmargin)
{
    ...
    for (;;) {
        ...
        case EOP:
            do_eop();
            if (linear) {
                if ((opcode = tt_dpx_get_unsigned_byte(dvi_handle)) == POST)
                    check_postamble();
                else
