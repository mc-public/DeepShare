...
read_res__data (ximage_info *info, FILE *fp, unsigned int size)
{
  ...
  VR_N = dpx_get_unsigned_pair(fp);
  VR_D = dpx_get_unsigned_pair(fp);
  HR_N = dpx_get_unsigned_pair(fp);
  HR_D = dpx_get_unsigned_pair(fp);
  VR_E = dpx_get_unsigned_byte(fp);
  HR_E = dpx_get_unsigned_byte(fp);
...
scan_cdef (ximage_info *info, int *smask, FILE *fp, unsigned int size)
{
  ...
  *smask = 0;
  ...
  N = dpx_get_unsigned_pair(fp);
  if (size < N * 6 + 2) {
    dpx_warning("JPEG2000: Inconsistent N value in Channel Definition box.");
    return -1;
  }
  for (i = 0; i < N; i++) {
    Cn   = dpx_get_unsigned_pair(fp); /* Cn */
    Typ  = dpx_get_unsigned_pair(fp);
    Asoc = dpx_get_unsigned_pair(fp); /* must be 0 for SMask */
    if (Cn > N)
...
scan_jp2h (ximage_info *info, int *smask, FILE *fp, unsigned int size)
{
  ...
  while (size > 0 && !error) {
    ...
    case JP2_BOX_IHDR:
      ...
      info->num_components = dpx_get_unsigned_pair(fp);
      /* c = */ dpx_get_unsigned_byte(fp); /* BPC - 1 */
      /* c = */ dpx_get_unsigned_byte(fp); /* C: Compression type */
      /* c = */ dpx_get_unsigned_byte(fp); /* UnkC */
      /* c = */ dpx_get_unsigned_byte(fp); /* IPR */
      have_ihdr = 1;
      break;
    case JP2_BOX_RES_:
