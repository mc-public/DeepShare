...
do_preamble (FILE *fp)
{
  ...
  if (fgetc(fp) == 89) {
    /* Skip comment */
    skip_bytes(dpx_get_unsigned_byte(fp), fp);
...
read_pk_char_header (struct pk_header_ *h, unsigned char opcode, FILE *fp)
{
  ...
  if ((opcode & 4) == 0) { /* short */
    h->pkt_len = (opcode & 3) << 8 | dpx_get_unsigned_byte(fp);
    h->chrcode = dpx_get_unsigned_byte(fp);
    h->wd = get_unsigned_triple(fp);     /* TFM width */
    h->dx = dpx_get_unsigned_byte(fp) << 16; /* horizontal escapement */
    h->dy = 0;
    h->bm_wd    = dpx_get_unsigned_byte(fp);
    h->bm_ht    = dpx_get_unsigned_byte(fp);
    ...
    h->pkt_len = (opcode & 3) << 16 | dpx_get_unsigned_pair(fp);
    h->chrcode = dpx_get_unsigned_byte(fp);
    h->wd = get_unsigned_triple(fp);
    h->dx = dpx_get_unsigned_pair(fp) << 16;
    h->dy = 0;
    h->bm_wd    = dpx_get_unsigned_pair(fp);
    h->bm_ht    = dpx_get_unsigned_pair(fp);
