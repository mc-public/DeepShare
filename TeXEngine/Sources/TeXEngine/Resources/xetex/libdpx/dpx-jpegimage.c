...
int
dpx_check_for_jpeg (rust_input_handle_t handle)
{
...
read_APP14_Adobe (struct JPEG_info *j_info, rust_input_handle_t handle)
{
    ...
    app_data->version   = tt_dpx_get_unsigned_pair(handle);
    app_data->flag0     = tt_dpx_get_unsigned_pair(handle);
    app_data->flag1     = tt_dpx_get_unsigned_pair(handle);
    app_data->transform = tt_dpx_get_unsigned_byte(handle);
...
read_APP0_JFIF (struct JPEG_info *j_info, rust_input_handle_t handle)
{
    ...
    app_data->version    = tt_dpx_get_unsigned_pair(handle);
    app_data->units      = tt_dpx_get_unsigned_byte(handle);
    app_data->Xdensity   = tt_dpx_get_unsigned_pair(handle);
    app_data->Ydensity   = tt_dpx_get_unsigned_pair(handle);
    app_data->Xthumbnail = tt_dpx_get_unsigned_byte(handle);
    app_data->Ythumbnail = tt_dpx_get_unsigned_byte(handle);
...
{
    tt_dpx_get_unsigned_byte(handle);
    /* Extension Code:
     *
...
read_APP2_ICC (struct JPEG_info *j_info, rust_input_handle_t handle, size_t length)
{
    ...
    app_data->seq_id      = tt_dpx_get_unsigned_byte(handle); /* Starting at 1 */
    app_data->num_chunks  = tt_dpx_get_unsigned_byte(handle);
...
#define COPY_CHUNK(f,s,l) while ((l) > 0) {                             \
    ...
    while (!found_SOFn && count < MAX_COUNT &&
        ...
        if ( marker == JM_SOI  ||
            ...
            length = tt_dpx_get_unsigned_pair(handle) - 2;
            switch (marker) {
...
JPEG_scan_file (struct JPEG_info *j_info, rust_input_handle_t handle)
{
    ...
    while (!found_SOFn &&
        ...
        if ( marker != JM_SOI  &&
             (marker  < JM_RST0 || marker > JM_RST7)) {
            int length = tt_dpx_get_unsigned_pair(handle) - 2;
            switch (marker) {
            case JM_SOF0:  case JM_SOF1:  case JM_SOF2:  case JM_SOF3:
            case JM_SOF5:  case JM_SOF6:  case JM_SOF7:  case JM_SOF9:
            case JM_SOF10: case JM_SOF11: case JM_SOF13: case JM_SOF14:
            case JM_SOF15:
                j_info->bits_per_component = tt_dpx_get_unsigned_byte(handle);
                j_info->height             = tt_dpx_get_unsigned_pair(handle);
                j_info->width              = tt_dpx_get_unsigned_pair(handle);
                j_info->num_components     = tt_dpx_get_unsigned_byte(handle);
                found_SOFn = 1;
                break;
            case JM_APP0:
                if (length > 5) {
