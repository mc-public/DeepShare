...
unsigned char dpx_get_unsigned_byte (FILE *file)
{
  int ch;
  if ((ch = fgetc (file)) < 0)
...
{
  while (n-- > 0)
    dpx_get_unsigned_byte(file);
}

signed char get_signed_byte (FILE *file)
{
  int byte;
  byte = dpx_get_unsigned_byte(file);
  if (byte >= 0x80)
    byte -= 0x100;
  return (signed char) byte;
}

unsigned short dpx_get_unsigned_pair (FILE *file)
{
  unsigned short pair = dpx_get_unsigned_byte(file);
  pair = (pair << 8) | dpx_get_unsigned_byte(file);
  return pair;
}

unsigned short sdpx_get_unsigned_pair (unsigned char *s)
{
...
unsigned int get_unsigned_triple(FILE *file)
{
  ...
  for (i=0; i<3; i++) {
    triple = (triple << 8) | dpx_get_unsigned_byte(file);
  }
  return triple;
}
...
int32_t get_signed_quad(FILE *file)
{
  ...
  for (i=0; i<3; i++) {
    quad = (quad << 8) | dpx_get_unsigned_byte(file);
  }
  return quad;
}
...
{
  uint32_t val = dpx_get_unsigned_byte (file);
  switch (num) {
  case 3: if (val > 0x7f)
            val -= 0x100;
          val = (val << 8) | dpx_get_unsigned_byte (file);
  case 2: val = (val << 8) | dpx_get_unsigned_byte (file);
  case 1: val = (val << 8) | dpx_get_unsigned_byte (file);
  default: break;
  }
  return val;
}
...
unsigned char
tt_dpx_get_unsigned_byte (rust_input_handle_t handle)
{
    int ch;
...
tt_get_signed_byte (rust_input_handle_t handle)
{
    ...
    byte = tt_dpx_get_unsigned_byte(handle);
    if (byte >= 0x80)
        byte -= 0x100;
...
unsigned short
tt_dpx_get_unsigned_pair (rust_input_handle_t handle)
{
    unsigned short pair = tt_dpx_get_unsigned_byte(handle);
    pair = (pair << 8) | tt_dpx_get_unsigned_byte(handle);
    return pair;
}
...
tt_get_unsigned_quad(rust_input_handle_t handle)
{
    ...
    for (i = 0; i < 4; i++)
        quad = (quad << 8) | tt_dpx_get_unsigned_byte(handle);
...
{
    uint32_t val = tt_dpx_get_unsigned_byte (handle);

    switch (num) {
    case 3:
        if (val > 0x7F)
            val -= 0x100;
        val = (val << 8) | tt_dpx_get_unsigned_byte (handle);
        /* fall through */
    case 2:
        val = (val << 8) | tt_dpx_get_unsigned_byte (handle);
        /* fall through */
