...
{
    if (tt_dpx_get_unsigned_byte (vf_handle) != PRE || tt_dpx_get_unsigned_byte (vf_handle) != VF_ID) {
        fprintf (stderr, "VF file may be corrupt\n");
        return;
    }

    /* skip comment */
    tt_skip_bytes (tt_dpx_get_unsigned_byte (vf_handle), vf_handle);
...
read_a_font_def(rust_input_handle_t vf_handle, int32_t font_id, int thisfont)
{
    ...
    dir_length = tt_dpx_get_unsigned_byte (vf_handle);
    name_length = tt_dpx_get_unsigned_byte (vf_handle);
...
process_vf_file (rust_input_handle_t vf_handle, int thisfont)
{
    ...
    while (!eof) {
        code = tt_dpx_get_unsigned_byte (vf_handle);
        ...
        default:
            if (code < 242) {
                ...
                uint32_t ch = tt_dpx_get_unsigned_byte (vf_handle);
