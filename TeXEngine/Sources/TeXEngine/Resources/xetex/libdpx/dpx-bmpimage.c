...
int
dpx_check_for_bmp (rust_input_handle_t handle)
{
                  ...
                  int  width, int  height, rust_input_handle_t handle)
{
    ...
    for (v = 0, eoi = 0; v < height && !eoi; v++) {
        for (h = 0, eol = 0; h < width && !eol; ) {
            ...
            b0 = tt_dpx_get_unsigned_byte(handle);
            b1 = tt_dpx_get_unsigned_byte(handle);
            count += 2;
            ...
            if (b0 == 0x00) {
                ...
                case 0x01: /* EOI */
                    ...
                    break;
                case 0x02:
                    h += tt_dpx_get_unsigned_byte(handle);
                    v += tt_dpx_get_unsigned_byte(handle);
                    count += 2;
                    break;
                default:
                    h += b1;
            ...
            if (b0 == 0x00) {
                ...
                default:
                    ...
                    if (h % 2) { /* starting at hi-nib */
                        for (i = 0; i < nbytes; i++) {
                            b = tt_dpx_get_unsigned_byte(handle);
                            *p++ |= (b >> 4) & 0x0f;
                            *p    = (b << 4) & 0xf0;
                        }
                    } else {
