...
{
    tfm->wlenfile = tt_dpx_get_unsigned_pair(tfm_handle);

    tfm->wlenheader = tt_dpx_get_unsigned_pair(tfm_handle);
    tfm->bc = tt_dpx_get_unsigned_pair(tfm_handle);
    tfm->ec = tt_dpx_get_unsigned_pair(tfm_handle);
    if (tfm->ec < tfm->bc)
        _tt_abort("TFM file error: ec(%u) < bc(%u) ???", tfm->ec, tfm->bc);

    tfm->nwidths  = tt_dpx_get_unsigned_pair(tfm_handle);
    tfm->nheights = tt_dpx_get_unsigned_pair(tfm_handle);
    tfm->ndepths  = tt_dpx_get_unsigned_pair(tfm_handle);
    tfm->nitcor   = tt_dpx_get_unsigned_pair(tfm_handle);
    tfm->nlig     = tt_dpx_get_unsigned_pair(tfm_handle);
    tfm->nkern    = tt_dpx_get_unsigned_pair(tfm_handle);
    tfm->nextens  = tt_dpx_get_unsigned_pair(tfm_handle);
    tfm->nfonparm = tt_dpx_get_unsigned_pair(tfm_handle);
...
ofm_do_char_info_zero (rust_input_handle_t ofm_handle, struct tfm_font *tfm)
{
    ...
    if (num_chars != 0) {
        ...
        for (i = 0; i < num_chars; i++) {
            tfm->width_index [i] = tt_dpx_get_unsigned_pair(ofm_handle);
            tfm->height_index[i] = tt_dpx_get_unsigned_byte(ofm_handle);
            tfm->depth_index [i] = tt_dpx_get_unsigned_byte(ofm_handle);
...
ofm_do_char_info_one (rust_input_handle_t ofm_handle, struct tfm_font *tfm)
{
    ...
    if (num_chars != 0) {
        ...
        for (i = 0; i < num_chars && char_infos_read < num_char_infos; i++) {
            ...
            /* Ignore next quad */
            tt_skip_bytes(4, ofm_handle);
            repeats = tt_dpx_get_unsigned_pair(ofm_handle);

            /* Skip params */
            for (j = 0; j < tfm->npc; j++)
                tt_dpx_get_unsigned_pair(ofm_handle);
