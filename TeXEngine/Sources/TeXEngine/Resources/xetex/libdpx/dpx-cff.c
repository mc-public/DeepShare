...
get_unsigned (rust_input_handle_t handle, int n)
{
    ...
    while (n-- > 0)
        v = v*0x100u + tt_dpx_get_unsigned_byte(handle);
...
cff_font *cff_open(rust_input_handle_t handle, int offset, int n)
{
    ...
    cff_seek_set(cff, 0);
    cff->header.major    = tt_dpx_get_unsigned_byte(cff->handle);
    cff->header.minor    = tt_dpx_get_unsigned_byte(cff->handle);
    cff->header.hdr_size = tt_dpx_get_unsigned_byte(cff->handle);
    cff->header.offsize  = tt_dpx_get_unsigned_byte(cff->handle);
    ...
    cff->num_glyphs = tt_dpx_get_unsigned_pair(cff->handle);
...
cff_get_index_header (cff_font *cff)
{
    ...
    idx->count = count = tt_dpx_get_unsigned_pair(cff->handle);
    if (count > 0) {
        idx->offsize = tt_dpx_get_unsigned_byte(cff->handle);
...
int cff_read_encoding (cff_font *cff)
{
    ...
    encoding->format = tt_dpx_get_unsigned_byte(cff->handle);
    length = 1;

    switch (encoding->format & (~0x80)) {
    case 0:
        encoding->num_entries = tt_dpx_get_unsigned_byte(cff->handle);
        (encoding->data).codes = NEW(encoding->num_entries, card8);
        for (i=0;i<(encoding->num_entries);i++) {
            (encoding->data).codes[i] = tt_dpx_get_unsigned_byte(cff->handle);
        }
    ...
    case 1:
    {
        ...
        for (i=0;i<(encoding->num_entries);i++) {
            ranges[i].first = tt_dpx_get_unsigned_byte(cff->handle);
            ranges[i].n_left = tt_dpx_get_unsigned_byte(cff->handle);
        }
    ...
    if ((encoding->format) & 0x80) {
        cff_map *map;
        encoding->num_supps = tt_dpx_get_unsigned_byte(cff->handle);
        encoding->supp = map = NEW(encoding->num_supps, cff_map);
        for (i=0;i<(encoding->num_supps);i++) {
            map[i].code = tt_dpx_get_unsigned_byte(cff->handle);
            map[i].glyph = tt_dpx_get_unsigned_pair(cff->handle); /* SID */
        }
...
int cff_read_charsets (cff_font *cff)
{
    ...
    charset->format = tt_dpx_get_unsigned_byte(cff->handle);
    ...
    case 0:
        ...
        for (i=0;i<(charset->num_entries);i++) {
            charset->data.glyphs[i] = tt_dpx_get_unsigned_pair(cff->handle);
        }
        count = 0;
        break;
    case 1:
    {
        ...
        while (count > 0 && charset->num_entries < cff->num_glyphs) {
            ...
            ranges[charset->num_entries].first = tt_dpx_get_unsigned_pair(cff->handle);
            ranges[charset->num_entries].n_left = tt_dpx_get_unsigned_byte(cff->handle);
    ...
    case 2:
    {
        ...
        while (count > 0 && charset->num_entries < cff->num_glyphs) {
            ...
            ranges[charset->num_entries].first = tt_dpx_get_unsigned_pair(cff->handle);
            ranges[charset->num_entries].n_left = tt_dpx_get_unsigned_pair(cff->handle);
...
int cff_read_fdselect (cff_font *cff)
{
    ...
    fdsel->format = tt_dpx_get_unsigned_byte(cff->handle);
    ...
    case 0:
        ...
        for (i=0;i<(fdsel->num_entries);i++) {
            (fdsel->data).fds[i] = tt_dpx_get_unsigned_byte(cff->handle);
        }
    ...
    {
        cff_range3 *ranges;
        fdsel->num_entries = tt_dpx_get_unsigned_pair(cff->handle);
        fdsel->data.ranges = ranges = NEW(fdsel->num_entries, cff_range3);
        for (i=0;i<(fdsel->num_entries);i++) {
            ranges[i].first = tt_dpx_get_unsigned_pair(cff->handle);
            ranges[i].fd = tt_dpx_get_unsigned_byte(cff->handle);
        }
        if (ranges[0].first != 0)
            _tt_abort("Range not starting with 0.");
        if (cff->num_glyphs != tt_dpx_get_unsigned_pair(cff->handle))
