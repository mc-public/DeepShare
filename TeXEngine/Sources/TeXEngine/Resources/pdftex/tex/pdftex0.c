...
zprintcs ( integer p ) 
{
  ...
  else if ( ( ( p >= 26627 ) && ( p <= 30190 ) ) || ( p > eqtbtop ) ) 
  printesc ( 590 ) ;
	...
	if ( memtop - 2 != pagetail ) 
	{
	  ...
	  if ( pagecontents > 0 ) 
	  {
	    ...
	    while ( r != memtop ) {
	      ...
	      printint ( t ) ;
	      print ( 1407 ) ;
	      if ( eqtb [29389 + t ].cint == 1000 ) 
	      t = mem [r + 3 ].cint ;
	      else t = xovern ( mem [r + 3 ].cint , 1000 ) * eqtb [29389 + 
	      t ].cint ;
	      printscaled ( t ) ;
	      if ( mem [r ].hh.b0 == 1 ) 
	      {
...
zprintcmdchr ( quarterword cmd , halfword chrcode ) 
{
  ...
  case 73 : 
    ...
    printparam ( chrcode - 29277 ) ;
      ...
      printesc ( 545 ) ;
      printint ( chrcode - 29389 ) ;
    } 
    break ;
  case 74 : 
    if ( chrcode < 29935 ) 
    printlengthparam ( chrcode - 29901 ) ;
    else {
	
      printesc ( 582 ) ;
      printint ( chrcode - 29935 ) ;
    } 
    break ;
  case 45 : 
    printesc ( 592 ) ;
...
zshoweqtb ( halfword n ) 
{
  ...
  if ( n < 1 ) 
  printchar ( 63 ) ;
  else if ( ( n < 26628 ) || ( ( n > 30190 ) && ( n <= eqtbtop ) ) ) 
  {
    sprintcs ( n ) ;
    printchar ( 61 ) ;
...
zscansomethinginternal ( smallnumber level , boolean negative ) 
{
  ...
  case 89 : 
	...
	{case 0 : 
	  curval = eqtb [29389 + curval ].cint ;
	  break ;
	case 1 : 
	  curval = eqtb [29935 + curval ].cint ;
	  break ;
	case 2 : 
...
convtoks ( void ) 
{
  ...
  case 19 : 
    {
      bool = scankeyword ( 897 ) ;
      if ( scankeyword ( 898 ) ) 
      curval = 2 ;
      else if ( scankeyword ( 897 ) ) 
      curval = 1 ;
      else curval = 0 ;
      savescannerstatus = scannerstatus ;
      savewarningindex = warningindex ;
...
openlogfile ( void ) 
{
  ...
  logopened = true ;
  {
    ...
    fprintf ( logfile , "%s%s%s",  "This is pdfTeX, Version 3.141592653" , "-2.6" ,     "-1.40.25" ) ;
    else
    fprintf ( logfile , "%s%s%s",  "This is pdfTeX, Version 3.141592653" , "-2.6" ,     "-1.40.25" ) ;
...
zoutwhat ( halfword p ) 
{
  ...
  case 2 : 
	...
	else if ( j < 16 ) 
	{
	  ...
	  aopenout ( writefile [j ]) ) promptfilename ( 1939 , 953 ) ;
	  writeopen [j ]= true ;
...
zdvishipout ( halfword p ) 
{
  ...
  synctexsheet ( eqtb [29294 ].cint ) ;
  {
    ...
    printchar ( 91 ) ;
    j = 9 ;
    while ( ( eqtb [29389 + j ].cint == 0 ) && ( j > 0 ) ) decr ( j ) ;
    {register integer for_end; k = 0 ;for_end = j ; if ( k <= for_end) do 
      {
	printint ( eqtb [29389 + k ].cint ) ;
	if ( k < j ) 
	printchar ( 46 ) ;
      } 
    while ( k++ < for_end ) ;} 
    ...
    29920 ].cint > 1073741823L ) || ( mem [p + 1 ].cint + eqtb [29919 ]
    .cint > 1073741823L ) ) 
    {
      {
	if ( interaction == 3 ) 
    ...
    if ( mem [p + 3 ].cint + mem [p + 2 ].cint + eqtb [29920 ].cint > 
    maxv ) 
    maxv = mem [p + 3 ].cint + mem [p + 2 ].cint + eqtb [29920 ].cint ;
    if ( mem [p + 1 ].cint + eqtb [29919 ].cint > maxh ) 
    maxh = mem [p + 1 ].cint + eqtb [29919 ].cint ;
    dvih = 0 ;
    dviv = 0 ;
    curh = eqtb [29919 ].cint ;
    dvif = 0 ;
    curhoffset = eqtb [29919 ].cint ;
    curvoffset = eqtb [29920 ].cint ;
    if ( eqtb [29924 ].cint != 0 ) 
    curpagewidth = eqtb [29924 ].cint ;
    else curpagewidth = mem [p + 1 ].cint + 2 * curhoffset + 2 * 4736286L ;
    if ( eqtb [29925 ].cint != 0 ) 
    curpageheight = eqtb [29925 ].cint ;
    ...
    if ( outputfilename == 0 ) 
    {
      ...
      openlogfile () ;
      packjobname ( 957 ) ;
      while ( ! bopenout ( dvifile ) ) promptfilename ( 958 , 957 ) ;
    ...
    {register integer for_end; k = 0 ;for_end = 9 ; if ( k <= for_end) do 
      dvifour ( eqtb [29389 + k ].cint ) ;
    while ( k++ < for_end ) ;} 
    dvifour ( lastbop ) ;
    lastbop = pageloc ;
    curv = mem [p + 3 ].cint + eqtb [29920 ].cint ;
    tempptr = p ;
    if ( mem [p ].hh.b0 == 1 ) 
    vlistout () ;
    else hlistout () ;
...
checkpdfversion ( void ) 
{
  ...
  if ( ! pdfversionwritten ) 
  {
    ...
    pdfosenable = true ;
      ...
      {
	pdfwarning ( 1020 , 1021 , true , true ) ;
	fixedpdfobjcompresslevel = 0 ;
      } 
      pdfosenable = false ;
    } 
...
ensurepdfopen ( void ) 
{
  ...
  while ( ! bopenout ( pdffile ) ) promptfilename ( 958 , 1025 ) ;
...
zadjustinterwordglue ( halfword p , halfword g ) 
{
  ...
  {
    pdfwarning ( 1085 , 1086 , true , true ) ;
    return ;
  } 
  c = 256 ;
  if ( ( p >= himemmin ) ) 
...
readexpandfont ( void ) 
{
  ...
  if ( ( pdffontstep [f ]!= 0 ) ) 
  {
    ...
    pdfwarning ( 1048 , 1099 , true , true ) ;
...
zletterspacefont ( halfword u , internalfontnumber f , integer e ) 
{
  ...
  pdfwarning ( 1101 , 1102 , true , true ) ;
  {register integer for_end; i = 0 ;for_end = nw - 1 ; if ( i <= for_end) do 
...
zvfdeffont ( internalfontnumber f ) 
{
  ...
  if ( k != 0 ) 
  {
    ...
    vflocalfontwarning ( f , k , 1114 ) ;
    if ( ds != fontdsize [k ]) 
    vflocalfontwarning ( f , k , 1115 ) ;
    if ( ( pdffontstep [f ]!= 0 ) ) 
...
zdovf ( internalfontnumber f ) 
{
  ...
  return ;
  stacklevel = 0 ;
  packfilename ( fontname [f ], 345 , 1108 ) ;
  if ( ! vfbopenin ( vffile ) ) 
  return ;
  if ( vfbyte () != 247 ) 
...
endlink ( void ) 
{
  ...
  -1073741824L ) ) 
  {
    ...
    if ( isshippingpage && matrixused () ) 
    {
      matrixrecalculate ( curh + eqtb [29926 ].cint ) ;
      mem [p + 1 ].cint = getllx () - eqtb [29926 ].cint ;
      mem [p + 2 ].cint = curpageheight - getury () - eqtb [29926 ].cint ;
      mem [p + 3 ].cint = geturx () + eqtb [29926 ].cint ;
      mem [p + 4 ].cint = curpageheight - getlly () + eqtb [29926 ].cint ;
    } 
    else mem [p + 3 ].cint = curh + eqtb [29926 ].cint ;
  } 
  poplinklevel () ;
} 
void 
...
endthread ( void ) 
{
  ...
  mem [lastthread + 4 ].cint = curv + eqtb [29928 ].cint ;
  if ( pdflastthreadnamedid ) 
...
zdodest ( halfword p , halfword parentbox , scaled x , scaled y ) 
{
  ...
  {
    warndestdup ( mem [p + 5 ].hh .v.RH , mem [p + 5 ].hh.b1 , 1869 , 1870 
    ) ;
    return ;
  } 
  objtab [k ].int4 = p ;
...
zpdfshipout ( halfword p , boolean shippingpage ) 
{
  ...
  if ( ( mem [p + 3 ].cint > 1073741823L ) || ( mem [p + 2 ].cint > 
  1073741823L ) || ( mem [p + 3 ].cint + mem [p + 2 ].cint + eqtb [29920 
  ].cint > 1073741823L ) || ( mem [p + 1 ].cint + eqtb [29919 ].cint > 
  1073741823L ) ) 
  {
    {
      if ( interaction == 3 ) 
  ...
  if ( mem [p + 3 ].cint + mem [p + 2 ].cint + eqtb [29920 ].cint > maxv 
  ) 
  maxv = mem [p + 3 ].cint + mem [p + 2 ].cint + eqtb [29920 ].cint ;
  if ( mem [p + 1 ].cint + eqtb [29919 ].cint > maxh ) 
  maxh = mem [p + 1 ].cint + eqtb [29919 ].cint ;
  ...
  if ( ! shippingpage ) 
  {
    ...
    curhoffset = eqtb [29922 ].cint + eqtb [29919 ].cint ;
    curvoffset = eqtb [29923 ].cint + eqtb [29920 ].cint ;
    if ( eqtb [29924 ].cint != 0 ) 
    curpagewidth = eqtb [29924 ].cint ;
  ...
  if ( ! shippingpage ) 
  {
    ...
    pdfindirectln ( 1151 , pdflastresources ) ;
  } 
  pdfbeginstream () ;
  if ( shippingpage ) 
  {
  ...
  if ( shippingpage ) 
  {
    ...
    pdfindirectln ( 1170 , pdflastpages ) ;
...
zpdffixdest ( integer k ) 
{
  ...
  if ( objtab [k ].int4 != -268435455L ) 
  return ;
  pdfwarning ( 1201 , 345 , true , false ) ;
  if ( objtab [k ].int0 < 0 ) 
  {
    print ( 1202 ) ;
...
zpdffixstructdest ( integer k ) 
{
  ...
  if ( objtab [k ].int4 != -268435455L ) 
  return ;
  pdfwarning ( 1206 , 345 , false , false ) ;
  if ( objtab [k ].int0 < 0 ) 
  {
    print ( 1202 ) ;
    print ( - (integer) objtab [k ].int0 ) ;
...
pdfprintinfo ( void ) 
{
  ...
  if ( pdfinfotoks != -268435455L ) 
  {
    ...
    creatorgiven = substrofstr ( 1225 , s ) ;
    producergiven = substrofstr ( 1226 , s ) ;
    creationdategiven = substrofstr ( 1227 , s ) ;
    moddategiven = substrofstr ( 1228 , s ) ;
    trappedgiven = substrofstr ( 1229 , s ) ;
  } 
  if ( ! producergiven ) 
  {
    pdfprint ( 1234 ) ;
  ...
  {
    pdfstrentryln ( 1233 , pdftexbanner ) ;
  } 
  pdfenddict () ;
} 
void 
...
zhpack ( halfword p , scaled w , smallnumber m ) 
{
  ...
  else if ( x > 0 ) 
  {
    ...
    ].hh .v.RH != -268435455L ) ) 
    {
      ...
      eqtb [29303 ].cint < 100 ) ) 
      {
	if ( ( eqtb [29917 ].cint > 0 ) && ( - (integer) x - totalshrink [0 
	]> eqtb [29909 ].cint ) ) 
	{
	  while ( mem [q ].hh .v.RH != -268435455L ) q = mem [q ].hh .v.RH 
	  ;
	  mem [q ].hh .v.RH = newrule () ;
	  mem [mem [q ].hh .v.RH + 1 ].cint = eqtb [29917 ].cint ;
	} 
	println () ;
	printnl ( 1268 ) ;
...
zappendtovlist ( halfword b ) 
{
  ...
  scaled d  ;
  halfword p  ;
  if ( curlist .auxfield .cint > eqtb [29933 ].cint ) 
  {
    d = mem [eqtb [26629 ].hh .v.RH + 1 ].cint - curlist .auxfield .cint - 
    mem [b + 3 ].cint ;
    if ( d < eqtb [29903 ].cint ) 
    p = newparamglue ( 0 ) ;
    else {
...
zmakescripts ( halfword q , scaled delta ) 
{
  ...
  if ( mem [q + 2 ].hh .v.RH == 0 ) 
  {
    ...
    mem [x + 1 ].cint = mem [x + 1 ].cint + eqtb [29913 ].cint ;
    ...
    mem [x + 4 ].cint = - (integer) shiftup ;
      ...
      mem [y + 1 ].cint = mem [y + 1 ].cint + eqtb [29913 ].cint ;
...
zmakeleftright ( halfword q , smallnumber style , scaled maxd , scaled maxh ) 
{
  ...
  delta2 = delta1 + delta1 - eqtb [29911 ].cint ;
  if ( delta < delta2 ) 
  delta = delta2 ;
...
zpostlinebreak ( boolean d ) 
{
  ...
  do {
    ...
    if ( eqtb [29931 ].cint != eqtb [29933 ].cint ) 
    mem [justbox + 3 ].cint = eqtb [29931 ].cint ;
    if ( eqtb [29932 ].cint != eqtb [29933 ].cint ) 
    mem [justbox + 2 ].cint = eqtb [29932 ].cint ;
    if ( ( eqtb [29929 ].cint != eqtb [29933 ].cint ) && ( curline == 
    curlist .pgfield + 1 ) ) 
    mem [justbox + 3 ].cint = eqtb [29929 ].cint ;
    if ( ( eqtb [29930 ].cint != eqtb [29933 ].cint ) && ( curline + 1 == 
    bestline ) ) 
    mem [justbox + 2 ].cint = eqtb [29930 ].cint ;
...
zvsplit ( halfword n , scaled h ) 
{
  ...
  q = vertbreak ( mem [v + 5 ].hh .v.RH , h , eqtb [29907 ].cint ) ;
  p = mem [v + 5 ].hh .v.RH ;
  if ( p == q ) 
  ...
  Result = vpackage ( p , h , 0 , eqtb [29907 ].cint ) ;
  return Result ;
} 
void 
printtotals ( void ) 
...
zfreezepagespecs ( smallnumber s ) 
{
  ...
  pagecontents = s ;
  pagesofar [0 ]= eqtb [29905 ].cint ;
  pagemaxdepth = eqtb [29906 ].cint ;
  pagesofar [7 ]= 0 ;
  pagesofar [1 ]= 0 ;
  pagesofar [2 ]= 0 ;
  pagesofar [3 ]= 0 ;
...
buildpage ( void ) 
{
  ...
  do {
	...
	if ( mem [r ].hh.b1 != n ) 
	{
	  ...
	  if ( eqtb [29389 + n ].cint == 1000 ) 
	  h = mem [r + 3 ].cint ;
	  else h = xovern ( mem [r + 3 ].cint , 1000 ) * eqtb [29389 + n ]
	  .cint ;
	...
	insertpenalties = insertpenalties + mem [p + 1 ].cint ;
	  ...
	  if ( eqtb [29389 + n ].cint == 1000 ) 
	  h = mem [p + 3 ].cint ;
	  else h = xovern ( mem [p + 3 ].cint , 1000 ) * eqtb [29389 + n ]
	  .cint ;
	  if ( ( ( h <= 0 ) || ( h <= delta ) ) && ( mem [p + 3 ].cint + mem 
	  [r + 3 ].cint <= eqtb [29935 + n ].cint ) ) 
	  {
	    ...
	    if ( eqtb [29389 + n ].cint <= 0 ) 
	    w = 1073741823L ;
	    else {
		
	      w = pagesofar [0 ]- pagesofar [1 ]- pagesofar [7 ];
	      if ( eqtb [29389 + n ].cint != 1000 ) 
	      w = xovern ( w , eqtb [29389 + n ].cint ) * 1000 ;
	    } 
	    if ( w > eqtb [29935 + n ].cint - mem [r + 3 ].cint ) 
	    w = eqtb [29935 + n ].cint - mem [r + 3 ].cint ;
...
itsallover ( void ) 
{
  ...
  if ( privileged () ) 
  {
    ...
    mem [curlist .tailfield + 1 ].cint = eqtb [29904 ].cint ;
    {
      prevtail = curlist .tailfield ;
...
znewgraf ( boolean indented ) 
{
  ...
  if ( indented ) 
  {
    ...
    mem [curlist .tailfield + 1 ].cint = eqtb [29901 ].cint ;
...
initmath ( void ) 
{
  ...
  if ( ( curcmd == 3 ) && ( curlist .modefield > 0 ) ) 
  {
    ...
    if ( eqtb [27158 ].hh .v.RH == -268435455L ) {
      ...
      if ( ( eqtb [29918 ].cint != 0 ) && ( ( ( eqtb [29318 ].cint >= 0 ) 
      && ( curlist .pgfield + 2 > eqtb [29318 ].cint ) ) || ( curlist 
      .pgfield + 1 < - (integer) eqtb [29318 ].cint ) ) ) 
      {
	l = eqtb [29904 ].cint - abs ( eqtb [29918 ].cint ) ;
	if ( eqtb [29918 ].cint > 0 ) 
	s = eqtb [29918 ].cint ;
	else s = 0 ;
      } 
    ...
    eqworddefine ( 29914 , w ) ;
    curlist .eTeXauxfield = j ;
    if ( ( eTeXmode == 1 ) ) 
    eqworddefine ( 29383 , x ) ;
    eqworddefine ( 29915 , l ) ;
    eqworddefine ( 29916 , s ) ;
...
aftermath ( void ) 
{
  ...
  if ( m < 0 ) 
  {
    {
      prevtail = curlist .tailfield ;
      mem [curlist .tailfield ].hh .v.RH = newmath ( eqtb [29902 ].cint , 
      0 ) ;
      curlist .tailfield = mem [curlist .tailfield ].hh .v.RH ;
    } 
    curmlist = p ;
...
zcheckpdfoutput ( strnumber s , boolean iserror ) 
{
  ...
  if ( eqtb [29342 ].cint <= 0 ) 
  {
    if ( iserror ) 
    pdferror ( s , 1788 ) ;
    else pdfwarning ( s , 1789 , true , true ) ;
  } 
} 
void 
scanpdfexttoks ( void ) 
...
zscaleimage ( integer n ) 
{
  ...
  if ( ( xr > 65535L ) || ( yr > 65535L ) ) 
  {
    xr = 0 ;
    yr = 0 ;
    pdfwarning ( 1787 , 1818 , true , true ) ;
  } 
...
scanimage ( void ) 
{
  ...
  {
    pdfwarning ( 1827 , 1828 , true , true ) ;
  ...
  {
    pdfwarning ( 1827 , 1829 , true , true ) ;
  ...
  if ( eqtb [29353 ].cint > 0 ) 
  {
    if ( warnpdfpagebox ) 
    {
      pdfwarning ( 1827 , 1830 , true , true ) ;
      warnpdfpagebox = false ;
    } 
...
zpdffixthread ( integer thread ) 
{
  pdffixthread_regmem 
  halfword a  ;
  pdfwarning ( 1835 , 1896 , true , false ) ;
  if ( objtab [thread ].int0 < 0 ) 
  {
    print ( 1202 ) ;
  ...
  if ( fixedpdfoutput > 0 ) 
  {
	...
	if ( fixedpdfdraftmode == 0 ) 
	{
	  ...
	  while ( objtab [k ].int4 == 0 ) {
	    ...
	    pdfwarning ( 1201 , 1208 , true , false ) ;
	    printint ( objtab [k ].int0 ) ;
	    print ( 1209 ) ;
	    println () ;
	    println () ;
	  ...
	  for_end) do 
		...
		pdfmarkchar ( i , j ) ;
	      ...
	      pdffontattr [i ], pdffontattr [k ]) ) 
	      {
		pdfwarning ( 1210 , 1211 , true , false ) ;
	  ...
	  do {
		...
		pdfprint ( 1214 ) ;
		{
	      ...
	      pdfintentryln ( 1196 , objtab [l ].int0 ) ;
	      if ( ! isroot ) 
	      pdfindirectln ( 1170 , pdflastpages ) ;
	      pdfprint ( 1215 ) ;
	      j = 0 ;
	  ...
	  if ( pdffirstoutline != 0 ) 
	  {
	    ...
	    pdfindirectln ( 1194 , pdffirstoutline ) ;
	    pdfindirectln ( 1195 , pdflastoutline ) ;
	    pdfintentryln ( 1196 , k ) ;
	    pdfenddict () ;
	    k = headtab [4 ];
	    while ( k != 0 ) {
		...
		pdflastoutline = k ;
	      ...
	      pdfbegindict ( k , 1 ) ;
	      pdfindirectln ( 1197 , pdfmem [objtab [k ].int4 ]) ;
	      pdfindirectln ( 65 , pdfmem [objtab [k ].int4 + 6 ]) ;
	      if ( pdfmem [objtab [k ].int4 + 1 ]!= 0 ) 
	      pdfindirectln ( 1170 , pdfmem [objtab [k ].int4 + 1 ]) ;
	      if ( pdfmem [objtab [k ].int4 + 2 ]!= 0 ) 
	      pdfindirectln ( 1198 , pdfmem [objtab [k ].int4 + 2 ]) ;
	      if ( pdfmem [objtab [k ].int4 + 3 ]!= 0 ) 
	      pdfindirectln ( 1199 , pdfmem [objtab [k ].int4 + 3 ]) ;
	      if ( pdfmem [objtab [k ].int4 + 4 ]!= 0 ) 
	      pdfindirectln ( 1194 , pdfmem [objtab [k ].int4 + 4 ]) ;
	      if ( pdfmem [objtab [k ].int4 + 5 ]!= 0 ) 
	      pdfindirectln ( 1195 , pdfmem [objtab [k ].int4 + 5 ]) ;
	      if ( objtab [k ].int0 != 0 ) 
	      pdfintentryln ( 1196 , objtab [k ].int0 ) ;
	  ...
	  pdfindirectln ( 1220 , pdflastpages ) ;
	  if ( threads != 0 ) 
	  pdfindirectln ( 1221 , threads ) ;
	  if ( outlines != 0 ) 
	  pdfindirectln ( 1222 , outlines ) ;
	  if ( namestree != 0 ) 
	  pdfindirectln ( 1223 , namestree ) ;
	  ...
	  if ( pdfcatalogopenaction != 0 ) 
	  pdfindirectln ( 1224 , pdfcatalogopenaction ) ;
	  pdfenddict () ;
	  ...
	  if ( pdfosenable ) 
	  {
	    ...
	    pdfindirectln ( 1245 , root ) ;
	    if ( eqtb [29376 ].cint == 0 ) 
	    pdfindirectln ( 1246 , objptr - 1 ) ;
	  ...
	  if ( ! pdfosenable ) 
	  {
	    ...
	    pdfprint ( 1248 ) ;
	    pdfintentryln ( 1242 , sysobjptr + 1 ) ;
	    pdfindirectln ( 1245 , root ) ;
	    if ( eqtb [29376 ].cint == 0 ) 
	    pdfindirectln ( 1246 , sysobjptr ) ;
	    if ( pdftrailertoks != -268435455L ) 
	    {
	      pdfprinttoksln ( pdftrailertoks ) ;
	      {
	...
	if ( fixedpdfdraftmode == 0 ) 
	bclose ( pdffile ) ;
	else pdfwarning ( 0 , 1200 , true , true ) ;
      } 
      if ( logopened ) 
      {
	putc ('\n',  logfile );
